...
class CustomListTest : CustomListActivityBase() {
    ...
    fun testClickOnBookByPosition() {
        var db = Database()
        Log.i("book", db.printAllBookIds().toString())

        Espresso.onData(CoreMatchers.anything()).atPosition(BookData.BOOK_ID_5).perform(ViewActions.click())
        Espresso.onView(ViewMatchers.withId(R.id.book_title)).check(ViewAssertions.matches(ViewMatchers.withText(BookData.BOOK_TITLE)))
        Espresso.onView(ViewMatchers.withId(R.id.book_author)).check(ViewAssertions.matches(ViewMatchers.withText(BookData.BOOK_AUTHOR)))
    }
